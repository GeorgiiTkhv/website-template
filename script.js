...
(function () {
  ...
  function renderHero() {
    ...
    if (trust) {
      trust.innerHTML = (DATA.hero?.trustBadges ?? [])
        .map((t) => `<span>${t}</span>`)
        .join("");
    }
  }
  ...
  function renderQuickQuote() {
    ...
    if (select) {
      select.innerHTML = (DATA.quickQuote?.services ?? [])
  ...
  // ---------- carousel (your logic, but as a function) ----------
...
/* =========================
   script.js (FULL)
   Requires: content.js loaded BEFORE this file
   Adds: form submit -> Node server (/api/quote)
   ========================= */
...
(function () {
  ...
  function setFormStatus(formEl, message, ok = true) {
    ...
    // If your forms are white (hero card), this keeps it readable:
    if (formEl.classList.contains("quote-box")) el.style.color = ok ? "rgba(11,19,36,.75)" : "rgba(160,0,0,.9)";
  }

  // API base (dev default)
  const API_BASE = "";
  ...
  function initQuoteForms() {
    ...
    async function handleSubmit(formType) {
      ...
      try {
        ...
        await postQuote({
          ...
          email,
          service,
          message: "" // you removed message field; keep for future
        });
    ...
    console.log("✅ Quote forms wired to", `${API_BASE}/api/quote`);
  }
